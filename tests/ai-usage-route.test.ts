...
describe("OpenAI usage summary", () => {
  it("groups token usage by call type and model without exposing conversation content", () => {
    const summary = summarizeUsage([
      ...
      { metadata: { call_type: "contextual_response", model: "gpt-5.4-mini", outcome: "turn_budget_blocked", input_tokens: 0, output_tokens: 0, total_tokens: 0 } },
    ...
    expect(summary.totals).toMatchObject({ calls: 5, provider_requests: 3, successful_requests: 2, blocked_attempts: 2, turn_budget_blocked: 1, input_tokens: 520, output_tokens: 120, total_tokens: 640, errors: 1 });
