...
describe("OpenAI usage summary", () => {
  it("groups token usage by call type and model without exposing conversation content", () => {
    ...
    expect(summary.totals).toMatchObject({ calls: 3, input_tokens: 520, output_tokens: 120, total_tokens: 640, errors: 1 });
    expect(summary.by_call_type.sales_response).toMatchObject({ calls: 2, input_tokens: 120, errors: 1 });
